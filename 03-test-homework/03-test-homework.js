...
function formatName() {
  ...
  const changeName = name[0].toUpperCase() + name.toLowerCase(1);
...
function salaryWithTax() {
  ...
  return `Ваша зарплата при учете налога 19.5% составит ${salaryWithTax} грн.`
}
...
function countLetter() {
  let word = prompt(`Введите слово, в котором хотите подсчитать количество букв.`);
  ...
  return `Буква ${letter} повторяется ${amount} раз.`;
}
...
function convertCurrency() {
  ...
  return `При обмене ${money} ${currencyUpper} получите ${onvertCurrency()}.`;
}
...
function deleteLetters() {
  ...
  return `Предложение без буквы ${letter}: ${newSentence}.`;
}

//10.
function isPalyndrom() {
  const palyndrom = prompt(`Введите палиндром.`);
  const newPalyndrom = palyndrom.toLowerCase().split(' ').join('');
  const reversePalyndrom = newPalyndrom.split(' ').reverse().join('');
...
function deleteDuplicateLetter() {
  ...
  return `Измененное предложение без повторяющихся букв: ${newSentence}.`;
}
