...
function getPowDigit() {
  ...
  const firstNumber = +prompt(`Введите число для возведения в степень.`);
  const secondNumber = +prompt(`Введите степень числа.`);
  let result = 1;
...
function deleteLetters() {
  ...
  let newSentence = sentence.replaceAll(letter, "");
...
function isPalyndrom() {
  ...
  return newPalyndrom === reversePalyndrom;
}
